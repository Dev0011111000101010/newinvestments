...
class Rcl_Custom_Fields {
	...
	function get_input( $field, $value = false ) {
		...
		$post_meta = get_post_meta($_GET['rcl-post-edit'], $field['name'].'_comment', true);
		...
		if($field['add_comment']) $content.='<div class="comment-to-input"><textarea placeholder="Краткий комментарий (не обязателен)" name="' . $field['name'] . '_comment"  id="" cols="30" rows="3">'.$post_meta.'</textarea></div>';
		$content.= '</div>';
    ...
    function get_type_uploadgoogledoc( $field ) {

	    $btn = $field['field-buttonname'] ?  $field['field-buttonname'] : 'Загрузить';
	    $fieldcontent = get_post_meta($_GET['rcl-post-edit'], $field['name'], true);
	    $input =  '<input value="'.$fieldcontent[0].'" id="'.$field['name'].'" type="url" ' . $pattern . ' ' . $this->maxlength . ' ' . $this->required . ' ' . $this->placeholder . ' ' . $this->get_class( $field ) . ' name="'.$field['name'].'[]" placeholder="Заполните если не работает кнопка загрузить" value="' . $this->value . '"/>';
	    $inputfilename = '<input value="'.$fieldcontent[1].'" type="text"'.$this->required.' placeholder="Имя файла" ' . $this->get_class( $field ) . ' name="'.$field['name'].'[]"/>';
	    $content= '<div class="repeater-single">';
	    $content.='<div class="left-btns">';
	    $content .= '<div class="button-upload_google btn btn-primary btn-primary1" onclick="f(this)"><span>'.$btn.'</span></div>
		<div class="btn btn-primary" onclick="previewFromFile(this);">Загрузить изображение ссылкой</div>';
		$content.= '</div><div class="float-left">'.$inputfilename.$input.'</div>';
	    $content.='</div>';
	    return $content;
    }
	function get_type_repeater( $field ) {
//		if ( get_post_meta( $_GET['rcl-post-edit'], 'repeaterdata', true ) ) {
//			$repeaterdata = get_post_meta( $_GET['rcl-post-edit'], 'repeaterdata', true );
//
//			for ($i=0; $i<=$repeaterdata; $i+=2) {
//
//			}
//		}

		$btn = $field['field-buttonname'] ?  $field['field-buttonname'] : 'Загрузить';
		$repeaterbtn = '<div class="addrepeaterfield">+</div>';
		//$description = $field['what-to-upload'];
		$content.='<div class="repeater-cont">';
		$input =  '<input id="'.$field['name'].'" type="hidden" ' . $pattern . ' ' . $this->maxlength . ' ' . $this->required . ' ' . $this->placeholder . ' ' . $this->get_class( $field ) . ' name="repeaterdata[]'.$field['name'].'[1][]" value="' . $this->value . '"/>';
		$inputfilename = '<input type="text" placeholder="Имя файла" ' . $this->get_class( $field ) . ' name="repeaterdata[]'.$field['name'].'[1][]"/>';
		$content.= '<div class="repeater-single">';
		if($field['drive_btn']) {
			$content .= '<div class="button-upload_google btn btn-primary" onclick="f(this)"><span>'.$btn.'</span></div><div class="float-right">'.$input.$inputfilename.$repeaterbtn.'</div>';
		}else {
	...
	function get_type_youtube_repeater( $field ) {

		$btn = 'Загрузить';
		$repeaterbtn = '<div class="addrepeaterfield">+</div>';
		if(!empty($this->value[0])) {
			foreach ($this->value as $l) {
				$content.='<div class="repeater-youtube-cont">';
				$input =  '<input id="'.$field['name'].'" type="url" ' . $pattern . ' ' . $this->maxlength . ' ' . $this->required . ' ' . $this->placeholder . ' ' . $this->get_class( $field ) . ' name="' . $field['name'] . '[]" value="' .$l. '"/>';
				$content.= '<div class="repeater-single">';
				$content .= '<div>'.$input.$repeaterbtn.'</div>';
				$content.='</div></div>';
			}
			return $content;
		}
		$content.='<div class="repeater-youtube-cont">';
		$input =  '<input id="'.$field['name'].'" type="url" ' . $pattern . ' ' . $this->maxlength . ' ' . $this->required . ' ' . $this->placeholder . ' ' . $this->get_class( $field ) . ' name="' . $field['name'] . '[]"/>';
		$content.= '<div class="repeater-single">';
			$content .= '<div>'.$input.$repeaterbtn.'</div>';
		$content.='</div></div>';
		return $content;
	}
	function get_type_googlethumb( $field ) {
		$btn = $field['field-buttonname'];
		$fieldcontent = get_post_meta($_GET['rcl-post-edit'], $field['name'], true);
		$iframe = '<iframe class="post-thumb" id="'.$field['name'].'" src="'.$fieldcontent.'"></iframe>';
		//$description = $field['what-to-upload'];
		$input =  '<input id="'.$field['name'].'" type="text" ' . $pattern . ' ' . $this->maxlength . ' ' . $this->required . ' ' . $this->placeholder . ' ' . $this->get_class( $field ) . ' name="'.$field['name'].'" value="' . $fieldcontent . '"/>';
		$content.= '<div class="repeater-single thumbail-field">';
		$content.= $iframe;
		$content .= '<div class="button-upload_google btn btn-primary" onclick="thumbnailUploader(this)"><span>'.$btn.'</span></div>'.$input;
		$content.='</div>';
		return $content;
	}
	...
	function get_type_number( $field ) {
		...
		$placeholder = $field['placeholder'] ? $field['placeholder'] : 'placeholder="Допустимо любое положительное значение или ноль."';
		return '<input onkeypress="return isNumm(event, this)" oninput="return isNumberKey(event, this)" type="text" ' . $min . ' ' . $max . ' ' . $this->required . ' ' . $placeholder . ' ' .'class="mask-numbers"'. ' name="' . $field['name'] . '" id="' . $this->field_id . '" maxlength="50" value="' . $this->value . '"/>';
	}
