...
class Rcl_Custom_Fields_Manager extends Rcl_Custom_Fields {
	...
	function get_field_types() {
		...
		$types = array(
			...
			'uploadgoogledoc'=> __( 'Загрузчик google drive файлов', 'wp-recall' ),
			'repeater'=> __( 'Repeater загрузка файлов', 'wp-recall' ),
			'googlethumb'=> __( 'Картинка thumbnail с гугла', 'wp-recall' ),
			'youtube_repeater'=> __( 'Repeater youtube video', 'wp-recall' ),
		...
		if ( in_array( $this->field['type'], $types ) ) {
			...
			if ( $this->field['type'] == 'file' ) {
	            ...
	            $options[] = array(
		            'type'	 => 'checkbox',
		            'slug'	 => 'add_comment',
		            'title'	 => __( 'Добавить поле пояснение', 'wp-recall' ),
		            'values' => array( 1 => __( 'Да', 'wp-recall' ) ),
	            );
            }
            else if ( $this->field['type'] == 'uploadgoogledoc' ) {


                $options[] = array(
                    'type'	 => 'text',
                    'slug'	 => 'field-buttonname',
            ...
            else if ( $this->field['type'] == 'googlethumb' ) {
            ...
            else if ( $this->field['type'] == 'repeater' ) {
	            ...
	            $options[] = array(
		            'type'	 => 'checkbox',
		            'slug'	 => 'drive_btn',
		            'title'	 => __( 'Использовать гугл драйв кнопку?', 'wp-recall' ),
		            'values' => array( 1 => __( 'Да', 'wp-recall' ) ),
	            );
            }
				...
				if ( in_array( $this->field['type'], array( 'select', 'radio' ) ) ) {
					...
					$options[] = array(
						'type'	 => 'checkbox',
						'slug'	 => 'add_comment',
						'title'	 => __( 'Добавить пояснение', 'wp-recall' ),
						'values' => array( 1 => __( 'Да', 'wp-recall' ) ),
					);
				}
