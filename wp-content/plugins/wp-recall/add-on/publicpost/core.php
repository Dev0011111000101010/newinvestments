...
function rcl_update_post_custom_fields( $post_id, $id_form = false ) {
	...
	if($_POST['thumbnail']) update_post_meta( $post_id, 'thumbnailimg', $_POST['thumbnail'] );
//	$needle = 'addedfilename';
//	$needle2 = 'addedfile';
//	$fieldstosave = [];
//	$counter = 1;
//	foreach ($_POST as $single=> $val) {
//		if(strpos($single, $needle)!==false) {
//			$lastletter = substr($single, -1);
//			$link = $_POST[$needle2.$lastletter];
//			empty($val) ? $val = $link : false;
//			$fieldstosave['addedfile'.$counter] = "$val::$link";
//			$counter++;
//		}
//	}
//	$needle = 'repeaterdata';

		if(!empty($_POST['repeaterdata']) && !is_null($_POST['repeaterdata'])) {
			update_post_meta( $post_id, 'repeaterdata', $_POST['repeaterdata'] );
		}
		foreach ($_POST as $single=> $val) {
			if(isset($_POST[$single.'_comment']) && !empty($_POST[$single.'_comment']) && !is_null($_POST[$single.'_comment'])) {
				$fieldname = $single."_comment";
				update_post_meta( $post_id, $fieldname , $_POST[$single.'_comment']);
			}
		}
}
