...
class Rcl_Public_Form extends Rcl_Public_Form_Fields {
	...
	function get_form() {
		...
		$content .= '<div class="tabs editor-mode-tabs">
<div class="s-tab tab1 active-tab">' . __( 'Простые настройки' ) . '</div>
<div class="s-tab tab2">' . __( 'Профессиональные настройки' ) . '</div>
</div>';
		...
		if ( $this->fields ) {
			...
			$i    = 0;
			$content .= '<div class="swiper-container top-publish-pagination">
  <div class="swiper-wrapper">
					<div class="swiper-slide"><div class="item active"><span class="top-num">1</span><span class="top-text">Шаг 1</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">2</span><span class="top-text">Шаг 2</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">3</span><span class="top-text">Шаг 3</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">4</span><span class="top-text">Шаг 4</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">5</span><span class="top-text">Шаг 5</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">6</span><span class="top-text">Шаг 6</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">7</span><span class="top-text">Шаг 7</span></div></div>
					<div class="swiper-slide"><div class="item"><span class="top-num">8</span><span class="top-text">Шаг 8</span></div></div>
					</div></div>
					<article class="form-page-wrap">';
			$contentarr = [];
				...
				if ( $this->taxonomies && in_array( $this->current_field['slug'], $taxField ) ) {
					...
					if ( in_array( $this->current_field['slug'], $defaultFields ) ) {
						...
						if ( $this->current_field['slug'] == 'post_content' ) {
							...
							$youtube   = $arraythis['current_field']['youtube'];
							$youtube == null ? $youtube = false : false;
							$contentField = displayYoutubeHelper( $youtube );
...
//				if ( $i == 1 ) {
//					$content .= '';
//					$content .= '<section class="form-page active" id="formpage' . $page . '">';
//				}
//				if ( $i % 14 == 0 ) {
//					$page    += 1;
//					$content .= '</section><section class="form-page" id="formpage' . $page . '">';
//				}
//				if ( count( $this->fields ) - ( $i * 14 ) < 14 ) {
//					echo '</article>';
//				}
				$fieldhtml .= '<div id="form-field-' . $this->current_field['slug'] . '" class="rcl-form-field field-' . $this->current_field['type'] . '">';
				$fieldhtml .= '<label>' . $CF->get_title( $this->current_field ) . ' ' . $required . '</label>';

				$fieldhtml .= $contentField;
				$currentnum = $this->current_field['pagenum'] + 1;

				$fieldhtml .= '</div>';

				$contentarr['page'.$currentnum][] = $fieldhtml;
			}
			for ($i = 1; $i < 9; $i++) {
				$content.= '<section class="form-page';
				if($i==1) $content.=' active';
				$content.= '" id="formpage' . $i . '">';
				foreach ($contentarr['page'.$i] as $item) {
					$content.=$item;
				}
				$content .= '</section>';
			}
...
//		$content .= '<nav id="form-pagination"><ul class="pagination">';
//
//		for ($c=1; $c<=$page; $c++) {
//			$content .= '<li class="page-item"><a class="page-link">'.$c.'</a></li>';
//		}
//
//		$content.= '</ul></nav>';
		...
		$content .= '<div class="steps-btns"><a href="" class="previous-step" style="display: none">Вернуться назад</a>';

		$content .= '<a href="" class="btn btn-primary next-step">Следующий шаг</a></div>';
		...
		$content .= '<a disabled="disabled" href="#" onclick="rcl_publish(this); return false;" id="rcl-publish-post" class="public-form-button  recall-button"><i class="rcli fa-print" aria-hidden="true"></i>' . __( 'Publish', 'wp-recall' ) . '</a>';
