...
class Rcl_List_Terms {
	...
	function get_checkbox_list( $term_ids = false ) {
		...
		foreach ( $terms_data as $term_id => $term ) {
			...
			$taxcountrieslist = ['countries', 'uchrediteli_countries', 'country_of_registration'];

			if(in_array(get_term( $term_id)->taxonomy, $taxcountrieslist)) {
				if ( $term['childrens'] ) {
					$options[] = '<div id="'.'id'.$term_id.'" class="child-list-category">'
					             . '<span class="parent-category rcl-field-input rcl-checkbox-box"><input class="parent-checkbox required-checkbox" id="'.'term-'.$term_id.'" type="checkbox"><label for="'.'term-'.$term_id.'" class="block-label">' . $term['name'] . '</label></span><div class="child-terms">'
				...
				if ( $term['childrens'] ) {
					$options[] = '<div id="'.'id'.$term_id.'" class="child-list-category">'
				            . '<span class="parent-category no-parent_checkbox">' . $term['name'] . '</span>'.'<div class="child-terms">'
				             . $this->get_checkbox_list( $term['childrens'] )
				             . '</div></div>';
					continue;
				}
